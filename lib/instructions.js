...
• String filters on free-text identifiers (e.g., project, chain, symbol) MUST be case-insensitive.
  – Prefer ILIKE with a prefix wildcard (e.g., project ILIKE 'uniswap%') rather than exact equality.
  – If using equality, normalize both sides (LOWER(column) = LOWER('value')).
• When converting a BIGINT epoch 'ts' to date, handle both seconds and milliseconds:
  (to_timestamp(CASE WHEN ts > 1000000000000 THEN ts/1000.0 ELSE ts END) AT TIME ZONE 'UTC')::date
...
• String filters on free-text identifiers (e.g., project, chain, symbol) MUST be case-insensitive.
  – Prefer ILIKE with a prefix wildcard (e.g., project ILIKE 'uniswap%') rather than exact equality.
  – If equality is used, normalize with LOWER(...)=LOWER(...).
• For BIGINT epoch timestamps named ts, assume seconds unless the value is too large;
  robust conversion:
  (to_timestamp(CASE WHEN ts > 1000000000000 THEN ts/1000.0 ELSE ts END) AT TIME ZONE 'UTC')::date
• Include ORDER BY for time series outputs.
