...
export function detectQueryIntent(question) {
  ...
  // Forecasting/prediction questions - route to general knowledge for analysis
  if (lowerQuestion.includes('forecast') || lowerQuestion.includes('predict') ||
      ...
      lowerQuestion.includes('what can i expect') || lowerQuestion.includes('based on') ||
      lowerQuestion.includes('cycle') || lowerQuestion.includes('performance around this time')) {
...
**🚨 TIMESTAMP COMPATIBILITY RULES (MANDATORY - CAUSES ERRORS IF IGNORED):**
...
• ❌ NEVER WRITE: ts >= MAX(ts) - 3600 (causes "operator does not exist" error)
• ✅ ALWAYS WRITE: ts >= MAX(ts) - INTERVAL '1 hour' (for update.* tables)
...
• update.cl_pool_hist: WHERE ts >= (SELECT MAX(ts) FROM update.cl_pool_hist) - INTERVAL '1 hour'
• update.lending_market_history: WHERE ts >= (SELECT MAX(ts) FROM update.lending_market_history) - INTERVAL '1 hour'

**CRITICAL EXAMPLES - ALWAYS USE INTERVAL FOR update.* TABLES:**
❌ BAD: WHERE ts >= (SELECT MAX(ts) FROM update.lending_market_history) - 3600
✅ GOOD: WHERE ts >= (SELECT MAX(ts) FROM update.lending_market_history) - INTERVAL '1 hour'
❌ BAD: WHERE ts >= (SELECT MAX(ts) FROM update.cl_pool_hist) - 86400  
✅ GOOD: WHERE ts >= (SELECT MAX(ts) FROM update.cl_pool_hist) - INTERVAL '1 day'
