...
export function detectQueryIntent(question) {
  ...
  // WALLET ANALYSIS INTENTS (new category)
  ...
  if (walletPatterns.some(pattern => pattern.test(lowerQuestion))) {
    return 'wallet_analysis';
  }
  
  // LIQUIDITY POOL INTENTS (new category)
  const lpPatterns = [
    /\blp\b.*\b(in|on|vs|versus|compare)\b/,
    /liquidity.*pool.*\b(vs|versus|compare)\b/,
    /\b(eth\/usdc|usdc\/eth|btc\/usdc|dai\/usdc).*\b(aerodrome|uniswap|curve|balancer)\b/,
    /compare.*yield.*\b(aerodrome|uniswap|curve|balancer)\b/,
    /expected.*yield.*\blp\b/,
    /\b(aerodrome|uniswap|curve|balancer).*vs.*\b(aerodrome|uniswap|curve|balancer)\b/
  ];
  
  if (lpPatterns.some(pattern => pattern.test(lowerQuestion))) {
    return 'liquidity_pool_analysis';
  }
  
  // ROTATION STRATEGY INTENTS (new category - these are actually answerable!)
  const rotationStrategyPatterns = [
    /rotate.*quarterly.*top.*decile/,
    /cagr.*rotated.*since/,
    /rotated.*into.*top.*\d+/,
    /quarterly.*rebalance.*top/
  ];
  
  if (rotationStrategyPatterns.some(pattern => pattern.test(lowerQuestion))) {
    return 'rotation_strategy';
  }
  
  // COMPLEX BACKTEST INTENTS (expanded - removed rotation strategies)
  const complexBacktestPatterns = [
    /backtest.*\b(borrow|loop|farm|leverage)\b/,
    /\b(borrow|loop).*\b(farm|yield|curve|aave)\b/,
    /\b(ltv|leverage).*\b(loop|farm|strategy)\b/,
    /\b(loop|leverage).*\d+x/
  ];
  
  if (complexBacktestPatterns.some(pattern => pattern.test(lowerQuestion))) {
    return 'complex_backtest';
  }
  
  // PORTFOLIO OPTIMIZATION INTENTS (route to general knowledge)
  const portfolioOptPatterns = [
    /optimal.*allocation/,
    /construct.*allocation/,
    /targeting.*sharpe/,
    /\b(sharpe|drawdown|volatility).*target/,
    /portfolio.*optim/
  ];
  
  if (portfolioOptPatterns.some(pattern => pattern.test(lowerQuestion))) {
...
export async function buildFilteredSchemaDoc(question) {
  ...
  if (fullSchemaDocCache && fullSchemaDocCache[cacheKey]) {
    return fullSchemaDocCache[cacheKey];
  }
  ...
  // Cache the result
  if (!fullSchemaDocCache) fullSchemaDocCache = {};
  fullSchemaDocCache[cacheKey] = result;
...
**TIMESTAMP COMPATIBILITY RULES (MANDATORY):**
• clean.* tables: ts is BIGINT (Unix timestamp) - use EXTRACT(EPOCH FROM date) for comparisons
• update.* tables: ts is TIMESTAMP WITH TIME ZONE - use direct date comparisons
• NEVER mix timestamp types in arithmetic operations (timestamp - integer)
• NEVER compare timestamp with time zone >= numeric (causes operator errors)
• Use EXTRACT(EPOCH FROM timestamp_field) to normalize to Unix timestamps when needed
• For time intervals: Use INTERVAL '90 days' with timestamps, or 7776000 (90*24*3600) with bigint
• For UNION operations: ALWAYS cast timestamps to same type using EXTRACT(EPOCH FROM ts) AS ts
• When in doubt, convert everything to Unix timestamps for compatibility
...
• update.cl_pool_hist: WHERE ts >= (SELECT MAX(ts) FROM table) - INTERVAL '1 hour'
• update.lending_market_history: WHERE ts >= (SELECT MAX(ts) FROM table) - INTERVAL '1 hour'
...
**COLUMN EXISTENCE RULES (MANDATORY):**
• ALWAYS check column existence before using in SELECT/WHERE clauses
• Common missing columns: exposure, il_risk, predictions, pool_meta (update.* only)
• If unsure about a column, use basic columns: symbol, project, chain, ts, price_usd, apy_base_supply
• For complex analysis, stick to one schema (either update.* OR clean.*) to avoid compatibility issues
...
3. **Avoid schema-specific columns**:
   - update.* only: id, url, exposure, il_risk, predictions, pool_meta
...
**Safe common columns for lending_market_history:**
symbol, project, chain, ts, apy_base_supply, apy_reward_supply, total_supply_usd, total_borrow_usd

**General rule:** When in doubt, use basic columns that exist in all schemas
...
export function extractWalletAddress(question) {
  const addressMatch = question.match(/0x[a-fA-F0-9]{40}/);
  return addressMatch ? addressMatch[0] : null;
}

export async function analyzeWalletWithAlchemy(walletAddress, apiKey) {
  try {
    let positionsData = null;
    let positionsError = null;
    
    // Try to get current token positions using production API
    try {
      const positionsResponse = await fetch(`https://api.g.alchemy.com/data/v1/${apiKey}/assets/tokens/by-address`, {
        method: 'POST',
        headers: {
      ...
      if (positionsResponse.ok) {
        ...
        // Try JSON-RPC method as fallback
        const rpcResponse = await fetch(`https://eth-mainnet.g.alchemy.com/v2/${apiKey}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (rpcData.result) {
          // Convert RPC format to REST API format
          positionsData = {
            data: [{
              tokens: rpcData.result.tokenBalances
                .filter(token => token.tokenBalance !== '0x0')
                .map(token => ({
                  contractAddress: token.contractAddress,
                  balance: parseInt(token.tokenBalance, 16),
                  symbol: 'Unknown', // Would need additional call to get metadata
                  name: 'Unknown Token'
                }))
            }]
          };
        } else {
          positionsError = `RPC method failed: ${rpcData.error?.message || 'Unknown error'}`;
        }
      }
    } catch (err) {
      positionsError = `Positions API error: ${err.message}`;
    }

    // Get transaction history (this should always work with proper API key)
    const historyResponse = await fetch(`https://eth-mainnet.g.alchemy.com/v2/${apiKey}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (historyData.error) {
      return { 
        error: `Alchemy API error: ${historyData.error.message}`,
    ...
    if (positionsData && !positionsError) {
      analysis = analyzeCurrentPositions(positionsData, transfers, walletAddress);
    } else {
      // Fall back to transaction-based analysis
      analysis = analyzeTransactionHistory(transfers, walletAddress);
    }
    
    return {
      wallet_address: walletAddress,
      ...analysis,
      positions_available: !positionsError,
      positions_error: positionsError,
      raw_transfers: transfers.slice(0, 5) // Include first 5 for debugging
    };
...
function analyzeCurrentPositions(positionsData, transfers, walletAddress) {
  ...
  if (positionsData && positionsData.data && positionsData.data.length > 0) {
    const addressData = positionsData.data[0]; // First address in response
    ...
    if (addressData.tokens && addressData.tokens.length > 0) {
      for (const token of addressData.tokens) {
        ...
        if (balance > 0) {
          ...
          totalPortfolioValueUSD += balanceUSD;
        }
      }
    }
  }
  
  // Process transaction history for activity context
  for (const transfer of transfers) {
    ...
    if (isOutgoing) outgoingCount++;
  }
  
  // Sort holdings by USD value
  currentHoldings.sort((a, b) => b.balance_usd - a.balance_usd);
  
  return {
    current_holdings: currentHoldings,
    total_portfolio_value_usd: totalPortfolioValueUSD,
    holdings_count: currentHoldings.length,
    largest_holding: currentHoldings[0] || null,
    transaction_breakdown: {
      incoming: incomingCount,
      outgoing: outgoingCount,
      total: transfers.length
    },
    recent_activity: transfers.slice(0, 3).map(t => ({
      hash: t.hash,
      asset: t.asset,
      value: t.value,
      from: t.from,
...
function analyzeTransactionHistory(transfers, walletAddress) {
  ...
  for (const transfer of transfers) {
    ...
    if (transfer.asset) {
      ...
      const tokenData = tokens.get(tokenKey);
      const amount = parseFloat(transfer.value) || 0;
      
      if (isIncoming) {
        tokenData.incoming += amount;
        tokenData.net_amount += amount;
      }
      if (isOutgoing) {
        tokenData.outgoing += amount;
        tokenData.net_amount -= amount;
      }
    }
    
    // Track protocols (basic heuristic)
    if (transfer.to && transfer.category === 'erc20') {
      protocols.add(transfer.to);
    }
    
    // Add USD value if available
    if (transfer.valueUSD) {
      totalValueUSD += parseFloat(transfer.valueUSD) || 0;
    }
  }
  
  const tokenArray = Array.from(tokens.entries()).map(([symbol, data]) => ({
    symbol,
    ...data
  }));
  ...
  return {
    ...
    most_active_token: findMostActiveToken(tokens),
    recent_activity: transfers.slice(0, 3).map(t => ({
      hash: t.hash,
      asset: t.asset,
      value: t.value,
    ...
    // For compatibility with current holdings format
...
function findMostActiveToken(tokens) {
  let mostActive = null;
  let maxTransactions = 0;
  
  for (const [symbol, data] of tokens) {
    const totalTransactions = (data.incoming > 0 ? 1 : 0) + (data.outgoing > 0 ? 1 : 0);
    if (totalTransactions > maxTransactions) {
      maxTransactions = totalTransactions;
      mostActive = { symbol, ...data };
    }
  }
...
export function buildRotationStrategyQuery(question, startYear = '2020') {
  // Extract key parameters from the question
  const lowerQuestion = question.toLowerCase();
  
  // Determine asset type
  let assetFilter = '';
  if (lowerQuestion.includes('stablecoin')) {
    assetFilter = 'AND stablecoin = true';
  } else if (lowerQuestion.includes('eth')) {
    assetFilter = "AND symbol ILIKE '%eth%'";
  } else if (lowerQuestion.includes('btc')) {
    assetFilter = "AND symbol ILIKE '%btc%'";
  }
  
  // Extract percentile (default to top decile = 90th percentile)
  let percentile = 90;
  const decileMatch = lowerQuestion.match(/top\s+(\d+)(?:th)?\s+percentile/);
  const decileWordMatch = lowerQuestion.match(/top\s+(decile|quintile|quartile)/);
  if (decileMatch) {
    percentile = parseInt(decileMatch[1]);
  } else if (decileWordMatch) {
    const word = decileWordMatch[1];
    if (word === 'decile') percentile = 90;
    else if (word === 'quintile') percentile = 80;
    else if (word === 'quartile') percentile = 75;
  }
  
  return {
    sql: `WITH quarterly_data AS (
      SELECT 
        DATE_TRUNC('quarter', TO_TIMESTAMP(ts)) AS quarter,
        symbol,
        project,
        chain,
        ...
        stablecoin
      FROM clean.lending_market_history 
      WHERE ts >= EXTRACT(EPOCH FROM '${startYear}-01-01'::date)
        AND apy_base_supply IS NOT NULL 
        AND apy_base_supply > 0
        AND apy_base_supply < 100  -- Sanity check
        ${assetFilter}
    ),
    quarterly_percentiles AS (
      SELECT 
        quarter,
        PERCENTILE_CONT(${percentile/100.0}) WITHIN GROUP (ORDER BY total_apy) AS percentile_threshold
      FROM quarterly_data
      GROUP BY quarter
    ),
    top_pools_per_quarter AS (
      SELECT 
        qd.quarter,
        AVG(qd.total_apy) AS avg_top_apy,
        COUNT(*) AS pool_count
      FROM quarterly_data qd
      JOIN quarterly_percentiles qp ON qd.quarter = qp.quarter
      WHERE qd.total_apy >= qp.percentile_threshold
      GROUP BY qd.quarter
      ORDER BY qd.quarter
    )
    SELECT 
      ...
      avg_top_apy,
      pool_count,
      EXTRACT(EPOCH FROM quarter) AS quarter_timestamp
    FROM top_pools_per_quarter
    ORDER BY quarter`
  };
}
...
export function calculateRotationStrategy(quarterlyData, initialAmount = 10000) {
  ...
  for (let i = 0; i < quarterlyData.length; i++) {
    ...
    const avgApy = parseFloat(quarter.avg_top_apy) || 0;
    const poolCount = parseInt(quarter.pool_count) || 0;
    
    // Quarterly return = APY / 4 (since APY is annual)
    const quarterlyReturn = avgApy / 4 / 100; // Convert percentage to decimal and divide by 4
    const quarterlyGain = currentValue * quarterlyReturn;
    currentValue = currentValue + quarterlyGain;
    
    results.push({
      quarter: quarter.quarter,
      apy: avgApy,
      pool_count: poolCount,
      quarterly_return: quarterlyReturn * 100,
      value: currentValue,
      gain: quarterlyGain
    });
  }
  ...
  // Calculate CAGR
  const startValue = initialAmount;
  const endValue = currentValue;
  const years = results.length / 4; // Number of quarters / 4
  const cagr = years > 0 ? (Math.pow(endValue / startValue, 1 / years) - 1) * 100 : 0;
  
  // Calculate average quarterly APY
  const avgQuarterlyApy = results.reduce((sum, r) => sum + r.apy, 0) / results.length;
  
  return {
    initial_amount: initialAmount,
    final_value: currentValue,
    total_return: currentValue - initialAmount,
    total_return_percent: ((currentValue - initialAmount) / initialAmount) * 100,
    cagr: cagr,
    years: years,
    quarters: results.length,
    avg_quarterly_apy: avgQuarterlyApy,
    avg_pool_count: results.reduce((sum, r) => sum + r.pool_count, 0) / results.length,
    quarterly_results: results.slice(-4), // Last 4 quarters for display
    start_date: results[0]?.quarter,
    end_date: results[results.length - 1]?.quarter
  };
}
...
export async function generateAnswerFromResults(openai, question, rows, presentationHint, intent = 'standard_query') {
  // Handle empty results with context-aware messages
  if (!rows || rows.length === 0) {
    const q = question.toLowerCase();
    
    // Provide helpful context for common empty result scenarios
    if (q.includes('tvl') && (q.includes('grew') || q.includes('fastest') || q.includes('category'))) {
      return "I couldn't find sufficient data for this time period. This might be due to recent schema changes or data availability. Try asking about a shorter time period or specific protocols.";
    }
    if (q.includes('backtest') || q.includes('simulation') || q.includes('portfolio')) {
      return "I don't have enough historical data to run this analysis. Try a simpler query with major tokens (BTC, ETH, USDC) over shorter time periods.";
    }
    if (q.includes('strategy') || q.includes('allocation') || q.includes('optimal') || q.includes('risk tolerance')) {
      return "This requires complex analysis beyond our current data scope. For investment strategies, consider consulting with a financial advisor.";
    }
    if (q.includes('inflow') || q.includes('outflow') || q.includes('net positive')) {
      return "I couldn't find recent flow data for this analysis. Try asking about specific protocol TVL changes or current rankings.";
    }
