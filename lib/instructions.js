...
export async function analyzeWalletWithAlchemy(walletAddress, apiKey) {
  try {
    ...
    if (data.error) {
      return { 
        error: `Alchemy API error: ${data.error.message}`,
    ...
    const analysis = analyzeTransfers(transfers, walletAddress);
...
function analyzeTransfers(transfers, walletAddress) {
  ...
  for (const transfer of transfers) {
    ...
    if (transfer.to && transfer.category === 'erc20') {
      // This is a simplified approach - in reality you'd want a contract address -> protocol mapping
  ...
  return {
    tokens: Array.from(tokens.entries()).map(([symbol, data]) => ({
      symbol,
      ...data
    })),
