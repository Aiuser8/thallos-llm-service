...
✅ WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.table_name) as FIRST condition
...
✅ WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM table) as FIRST condition
✅ LIMIT 100 (NEVER 5, 10, or 50)
...
CORE RULES:
• ⚠️ TIMESTAMP FILTER MANDATORY: ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM table)
...
✅ WHERE clause starts with: ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.cl_pool_hist)
