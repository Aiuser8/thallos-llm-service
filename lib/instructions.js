...
• Focus on education, not financial advice`;
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      max_completion_tokens: 600, // Concise educational responses
      temperature: 0.5, // Slightly higher for natural educational tone
    });
...
WITH latest_data AS (
  ...
  WHERE ts >= (SELECT MAX(ts) - 21600 FROM update.lending_market_history)
...
export async function planQuery(openai, question, doc = null, intent = 'standard_query') {
  ...
  try {
    ...
    messages: await buildPlannerMessages(question, doc),
      max_completion_tokens: 800, // SQL queries are typically short - limits unnecessary generation
      temperature: 0.1, // Low temp for consistent, deterministic SQL generation
  });
...
• Focus on answering core question`;
  ...
  try {
    ...
    response_format: { type: "json_object" },
      ...
      max_completion_tokens: 800, // SQL queries are short - limits unnecessary generation
      temperature: 0.1, // Low temp for deterministic SQL fixes
  });
    text = resp.choices?.[0]?.message?.content || "{}";
...
🎯 IMPORTANT: Don't just focus on the first few rows! Show variety across different chains and protocols to make your answer feel comprehensive and data-rich.`;
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: `Question: ${stripInvisibles(question)}\n\nQuery Results (JSON): ${JSON.stringify(rows).slice(0, 100000)}${timestampInfo}${diversityInfo}` },
      ],
      max_completion_tokens: 1200, // Reasonable limit for comprehensive answers without rambling
      temperature: 0.3, // Low temp for factual, data-driven responses
    });
...
export async function generateAnswer(openai, question, rows, presentationHint) {
  ...
  const sys = [
    ...
    "NO EMOJIS - keep responses professional and text-only.",
    "MANDATORY: ALWAYS include the data timestamp in your response (you'll be provided with the date).",
  ...
  const timestampMsg = dataDate ? `\nData Date: ${dataDate} - YOU MUST mention this in your response!` : '';
    ...
    messages: [
      ...
      { role: "user", content: `Question: ${stripInvisibles(question)}` },
      {
        ...
        content:
          ...
          `Rows (JSON): ${JSON.stringify(rows).slice(0, 100000)}${timestampMsg}`,
      },
    ],
    max_completion_tokens: 1200, // Reasonable limit for comprehensive answers
