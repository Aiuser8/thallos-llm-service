...
• update.token_price_daily: WHERE price_timestamp >= (SELECT MAX(price_timestamp) - INTERVAL '5 minutes' FROM update.token_price_daily)
• update.lending_market_history: WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.lending_market_history) ORDER BY total_supply_usd DESC, (apy_base_supply + COALESCE(apy_reward_supply, 0)) DESC
• update.cl_pool_hist: WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.cl_pool_hist) ORDER BY tvl_usd DESC, apy DESC
• clean.lending_market_history: WHERE ts >= (SELECT MAX(ts) - 21600 FROM clean.lending_market_history) ORDER BY total_supply_usd DESC
