...
function isoToEnglish(yyyy, mm, dd) {
  ...
  return `${month} ${d}${(['th','st','nd','rd'][((d%10)>3||(d%100)>=11&&(d%100)<=13)?0:d%10] || 'th')} ${y}`;
}
...
function extractIsoRange(text = '') {
  ...
  return `${monthTokenToEnglish(y1, m1)} ${Number(d1)}, ${y1}–${monthTokenToEnglish(y2, m2)} ${Number(d2)}, ${y2}`;
}
...
function humanizeDatesInText(out) {
  ...
  out = out.replace(
    ...
    (_m, y, mo, d) => `${monthTokenToEnglish(y, mo)} ${Number(d)}, ${y}`
  );

  out = out.replace(
    /\b(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])\s*(?:to|–|—|-)\s*(\d{4})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])\b/g,
    (_m, y1, m1, d1, y2, m2, d2) =>
      `${monthTokenToEnglish(y1, m1)} ${Number(d1)}, ${y1}–${monthTokenToEnglish(y2, m2)} ${Number(d2)}, ${y2}`
  );
...
function prependDateRangeIfAny(question = '', answer = '') {
  const re = /(\d{4}-\d{2}-\d{2})\s*(?:to|–|—|-)\s*(\d{4}-\d{2}-\d{2})/i;
  if (!re.test(question)) return answer;
  if (/\b(January|February|March|April|May|June|July|August|September|October|November|December)\b/.test(answer)) {
    return answer;
  }
  return `During ${question.replace(re, (_m, a, b) => `${a}–${b}`)}, ${answer.charAt(0).toLowerCase()}${answer.slice(1)}`;
}
