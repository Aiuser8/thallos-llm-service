...
• MUST include: WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.cl_pool_hist)
• MUST include: AND tvl_usd > 1000000 (filter low liquidity)
• MUST use: ORDER BY tvl_usd DESC, apy DESC (TVL FIRST!)
• For vague "best" queries: Prefer blue chips with filter above
...
🚨 NEVER RETURN OLD DATA: If you don't filter by timestamp, you'll get June data instead of September!
...
- MUST filter: ts >= (SELECT MAX(ts) - INTERVAL '6 hours'...)
...
8. Two tokens together = POOL query (not separate lending): "WETH-USDC", "WETH USDC" → use cl_pool_hist
