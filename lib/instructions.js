...
LIQUIDITY POOL YIELDS (CRITICAL - ALWAYS USE THIS PATTERN):
...
POOL YIELDS (ALWAYS USE THIS EXACT PATTERN):
  SELECT pool_id, symbol, project, chain, apy, tvl_usd, ts 
  FROM update.cl_pool_hist 
  WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.cl_pool_hist) 
  AND tvl_usd > 1000000
...
⚠️ CRITICAL: "best pools" = HIGH TVL FIRST, then APY
- MUST filter: ts >= (SELECT MAX(ts) - INTERVAL '6 hours'...) - ALWAYS!
- MUST filter: tvl_usd > 1000000 (no low liquidity pools)
- MUST filter: (project ILIKE '%aerodrome%' OR project ILIKE '%uniswap%') - unless user specifies protocol
- MUST order: ORDER BY tvl_usd DESC, apy DESC (TVL FIRST!)
...
6. Always include timestamp field in SELECT
7. Always filter out NULLs: AND value_column IS NOT NULL
8. Pool pairs: (symbol ILIKE '%WETH-USDC%' OR symbol ILIKE '%USDC-WETH%')
9. Case-insensitive: Use ILIKE for text matching
10. Two tokens together = POOL query (not separate lending): "WETH-USDC", "WETH USDC" → use cl_pool_hist

🎯 "BEST/GOOD/SOLID OPPORTUNITY" = HIGH TVL FIRST + REASONABLE APY:
...
• MANDATORY SQL PATTERN for "best pools":
  - WHERE tvl_usd > 1000000 (filter out low liquidity)
  - ORDER BY tvl_usd DESC, apy DESC (TVL FIRST, ALWAYS!)
