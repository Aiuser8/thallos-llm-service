...
LENDING OPPORTUNITIES (BALANCED APY + TVL):
...
🎯 KEY CHANGE: Order by APY first (not TVL first) to catch best opportunities
- Filter ensures safety: total_supply_usd > 1M minimum
- Higher LIMIT (50) ensures we don't miss opportunities further down
- APY DESC first = best yields shown first
- TVL DESC second = tiebreaker for equal APYs
...
• LIMIT 50 (not 10) - ensures we see best opportunities even if further down
...
- LIMIT 50 (not 10) to ensure we don't miss better opportunities further down
...
6. LIMIT 50 (not 10) - ensures we don't miss best opportunities further down in results
...
• MANDATORY SQL PATTERN for "best pools":
  ...
  - LIMIT 50 (not 10) to ensure we see opportunities further down
  - WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.cl_pool_hist) (recent data only!)
• This ensures: Safety through filtering + Best yields through ordering + Nothing missed through higher limit
...
• High-Risk Alternative: EXOTIC-TOKEN at 145% APY, 45% volatility ⚠️
