...
=== KEY RULES ===
1. SINGLE ASSET PRICE: When user asks for ONE token price, use symbol = 'BTC' (NOT symbol IN ('BTC', 'WBTC')) - NEVER return multiple versions
2. Balance APY + Safety: ORDER BY apy DESC, tvl_usd DESC (APY first!) + filter tvl_usd > 1M for safety
3. Blue chips first: ETH, BTC, USDC, USDT, DAI (safer, more liquid)
4. Default to Aave-V3 for lending unless user specifies otherwise
5. Default to Aerodrome/Uniswap for pools unless user specifies protocol
