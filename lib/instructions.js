...
• clean.* tables: ts is BIGINT (Unix timestamp)
• update.* tables: ts is TIMESTAMP WITH TIME ZONE
...
• For arbitrage/complex queries: Use ONLY update.* tables for real-time analysis
• For historical analysis: Use ONLY clean.* tables
...
• Token prices: INTERVAL '15 minutes' 
• Lending rates: INTERVAL '6 hours'
• Pool APYs: INTERVAL '6 hours'

🚨 CORE TABLE PATTERNS:
• update.token_price_daily: DISTINCT ON (symbol) ORDER BY symbol, price_timestamp DESC
• update.lending_market_history: WHERE ts >= MAX(ts) - INTERVAL '6 hours'
• update.cl_pool_hist: WHERE ts >= MAX(ts) - INTERVAL '6 hours'
...
export async function retryPlan(openai, question, previousSql, errMsg, doc = null, intent = 'standard_query', retryCount = 1) {
  ...
  // 📈 PROGRESSIVE LEARNING STRATEGY - Get smarter with each retry
  let retryStrategy = '';
  if (retryCount === 1) {
    // 🔧 FIRST RETRY: Learn from the specific error, fix intelligently
    switch (errorType) {
      case 'timestamp':
        retryStrategy = `TIMESTAMP ERROR DETECTED - Learn and fix:
        • NEVER mix update.* tables (TIMESTAMP WITH TIME ZONE) with clean.* tables (BIGINT)
        • For update.* tables: Use INTERVAL '6 hours' not integer subtraction
        • For clean.* tables: Use unix timestamp arithmetic
        • Choose ONE schema type and stick with it`;
        break;
      case 'union':
        retryStrategy = `UNION TYPE ERROR - Learn and fix:
        • NEVER use UNION ALL between different table types
        • Use json_build_object() to combine different result sets
        • Example: SELECT json_build_object('borrow', borrow_cte, 'pools', pool_cte)`;
        break;
      case 'schema':
        retryStrategy = `COLUMN/TABLE ERROR - Learn and fix:
        • Double-check all column names against the schema
        • Verify table names are spelled correctly
        • Use only columns that actually exist in the tables`;
        break;
      case 'type_mismatch':
        retryStrategy = `TYPE MISMATCH ERROR - Learn and fix:
        • Check data types in SELECT statements
        • Use proper casting with ::type or CAST()
        • Ensure consistent data types in operations`;
        break;
      default:
        retryStrategy = `GENERAL ERROR - Learn and simplify:
        • Reduce query complexity while keeping core logic
        • Focus on the main question being asked`;
    }
  } else if (retryCount === 2) {
    // 🔄 SECOND RETRY: Major simplification - learn that complex approaches failed
    retryStrategy = `COMPLEXITY REDUCTION - Previous attempts failed, learn to simplify:
    1. Use ONLY ONE table (prefer update.* for current data)
    2. Remove complex JOINs, CTEs, and subqueries  
    3. Use simple SELECT with basic WHERE conditions
    4. Focus on answering the core question only
    5. Add LIMIT 10 to prevent large result sets
    6. If asking about rates/APY, just find the top few results`;
  } else {
    // 🚨 THIRD+ RETRY: Emergency fallback - learn to be ultra-conservative
    retryStrategy = `EMERGENCY SIMPLIFICATION - Multiple failures, use safest approach:
    1. Single table SELECT only
    2. Most basic WHERE condition possible
    3. ORDER BY one simple column
    4. LIMIT 5
    5. Return whatever data exists, even if not perfect
    6. Focus on "something is better than nothing"`;
  }
...
export async function generateAnswerFromResults(openai, question, rows, presentationHint, intent = 'standard_query') {
  ...
  if (!rows || rows.length === 0) {
    ...
    if (q.includes('lending') || q.includes('borrow') || q.includes('apy') || q.includes('rates')) {
      return "I couldn't find data for that specific lending query. Try asking: 'What are the current lending opportunities?' or 'Where can I borrow ETH for the cheapest rates?'";
    }
    if (q.includes('pool') || q.includes('liquidity') || q.includes('farming')) {
      return "I couldn't find pool data for that query. Try asking: 'What are the current APYs for WETH-USDC pools?' or 'Show me high APY liquidity pools'";
    }
    if (q.includes('price') || q.includes('worth') || q.includes('cost')) {
      return "I couldn't find price data for that token. Try asking: 'What is the current price of ETH?' or ask about major tokens like BTC, ETH, USDC.";
    }
    if (q.includes('arbitrage') || q.includes('opportunities')) {
      return "I couldn't find arbitrage data for that specific query. Try asking: 'Where can I borrow USDC cheap and lend it for higher rates?'";
    }
