...
• Pattern: SELECT * FROM update.cl_pool_hist WHERE apy > X AND tvl_usd > Y ORDER BY apy DESC
...
• Simple pattern: SELECT pool_id, project, chain, symbol, tvl_usd, apy, apy_base, ts FROM update.cl_pool_hist WHERE apy > 100 AND tvl_usd > 2000000 ORDER BY apy DESC LIMIT 10
...
• ✅ CORRECT: Single table query from update.cl_pool_hist only
...
export async function retryPlan(openai, question, previousSql, errMsg, doc = null, intent = 'standard_query', retryCount = 1) {
  ...
  if (retryCount === 1) {
    ...
    switch (errorType) {
      case 'timestamp':
        retryStrategy = `TIMESTAMP ERROR - CRITICAL SCHEMA SEPARATION FIX:
        • This error indicates mixing update.* and clean.* tables with different timestamp types
        • SOLUTION: Use ONLY update.* tables for current queries
        • For high APY pools: SELECT * FROM update.cl_pool_hist WHERE apy > 100 AND tvl_usd > 2000000 ORDER BY apy DESC
        • NEVER mix schemas - always stick to one schema type per query
        • Remove any UNION, CTE combinations, or multi-schema references`;
        break;
      case 'union':
