...
export function detectQueryIntent(question) {
  ...
  if (lowerQuestion.includes('trend') || lowerQuestion.includes('trending') || 
      lowerQuestion.includes('momentum') || lowerQuestion.includes('moving average') ||
      lowerQuestion.includes('volatility') || lowerQuestion.includes('correlation')) {
...
function getRelevantTables(registry, keywords) {
  ...
  // 🚨 SCHEMA PREFERENCE SYSTEM - Prevent UNION by defaulting to update.* only
  ...
  for (const [fqtn, meta] of Object.entries(registry)) {
    // Only include tables from the preferred schema to prevent mixing
...
🚨 FORBIDDEN: NEVER use UNION or UNION ALL - this causes timestamp type errors!
🚨 FORBIDDEN: NEVER mix update.* and clean.* schemas in one query!
...
• **COMPARISONS**: Compare protocols side-by-side with JOIN queries within same schema
...
🧠 ADVANCED FINANCIAL ANALYTICS (NEW):
• **YIELD CURVE ANALYSIS**: Multi-timeframe yield analysis with LAG/LEAD functions
• **VOLATILITY METRICS**: STDDEV() for APY stability, coefficient of variation
• **RISK-ADJUSTED SCORING**: Sharpe-like ratios using AVG(apy)/STDDEV(apy)
• **MOMENTUM INDICATORS**: Moving averages with window functions
• **OUTLIER DETECTION**: Statistical flagging using 2-sigma rules
• **CORRELATION ANALYSIS**: CORR() function for cross-asset relationships
• **SEASONALITY PATTERNS**: EXTRACT() functions for time-based analysis
• **PERCENTILE ANALYSIS**: PERCENTILE_CONT() for distribution insights
...
🚨 CRITICAL SCHEMA MIXING RULE:
• HIGH APY POOLS: Use ONLY update.cl_pool_hist - NEVER mix with clean.cl_pool_hist
• **NEVER USE UNION**: Completely forbidden - always causes timestamp type errors
• **SINGLE TABLE APPROACH**: For ANY multi-condition query, use ONE table only
• Pattern: SELECT * FROM update.cl_pool_hist WHERE apy > X AND tvl_usd > Y ORDER BY tvl_usd DESC, apy DESC
• **NO EXCEPTIONS**: Even if you want comprehensive data, stick to one schema
...
🚨 ADVANCED FINANCIAL ANALYTICS ENGINE (MANDATORY):
• **YIELD CURVE ANALYSIS**: Calculate yield curves across different time periods
  - Pattern: SELECT symbol, apy, LAG(apy, 1) OVER (ORDER BY ts) as prev_apy, 
    (apy - LAG(apy, 1) OVER (ORDER BY ts)) as apy_change FROM table
  - Use window functions for trend analysis: ROW_NUMBER(), LAG(), LEAD()
• **RISK-ADJUSTED RETURNS**: Calculate Sharpe-like ratios and volatility metrics
  - Pattern: SELECT symbol, AVG(apy) as mean_apy, STDDEV(apy) as apy_volatility,
    AVG(apy) / NULLIF(STDDEV(apy), 0) as risk_adjusted_score FROM table
• **CORRELATION ANALYSIS**: Cross-asset yield correlations and market relationships
  - Use CORR() function: SELECT CORR(a.apy, b.apy) as correlation FROM table a JOIN table b
• **TREND DETECTION**: Moving averages and momentum indicators
  - Pattern: SELECT symbol, apy, AVG(apy) OVER (ORDER BY ts ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) as ma_7d
• **OUTLIER DETECTION**: Flag unusually high/low rates with statistical context
  - Pattern: WITH stats AS (SELECT AVG(apy) as mean, STDDEV(apy) as std FROM table)
    SELECT *, CASE WHEN ABS(apy - mean) > 2 * std THEN 'OUTLIER' END as flag
• **SEASONALITY PATTERNS**: Time-based yield pattern analysis
  ...
  if (retryCount === 1) {
    ...
    switch (errorType) {
      ...
      case 'complex_pool_query':
        retryStrategy = `COMPLEX POOL QUERY - SIMPLIFIED APPROACH:
        • This is a multi-condition liquidity pool query that needs simple AND logic
        • EXACT PATTERN: SELECT pool_id, symbol, project, chain, apy, tvl_usd, ts 
          FROM update.cl_pool_hist 
          WHERE ts >= (SELECT MAX(ts) - INTERVAL '6 hours' FROM update.cl_pool_hist) 
            AND apy > [APY_THRESHOLD] AND tvl_usd > [TVL_THRESHOLD] 
            AND apy IS NOT NULL AND tvl_usd IS NOT NULL 
          ORDER BY tvl_usd DESC, apy DESC LIMIT 20
        • DO NOT use UNION, CTEs, or complex joins for this type of query
        • DO NOT mix update.* and clean.* schemas
        • Use simple AND conditions in a single WHERE clause
...
• When creating tables, use clean markdown with proper alignment
• Add separators and spacing for readability
• Use emojis sparingly for visual appeal (📈 📊 💰 ⚠️ ✅ ❌)
• Format numbers with commas for large values (e.g., $1,234,567)
• Right-align numerical columns, left-align text columns
• Add table headers with clear, descriptive names
• Example format:
  | Pool Symbol        | APY     | TVL           | Risk Level |
  |--------------------|---------|---------------|------------|
  | WETH-USDC         | 12.45%  | $1,234,567    | ✅ Low     |
  | EXOTIC-TOKEN      | 245.67% | $12,345       | ⚠️ High    |
...
• **Statistical Interpretation**: Explain volatility (STDDEV), correlations, and risk metrics in practical terms
• **Trend Analysis**: Identify momentum, reversals, moving averages, and pattern significance  
• **Risk Assessment**: Interpret risk-adjusted scores, Sharpe ratios, and volatility measures
• **Outlier Analysis**: Explain why outliers occur, their statistical significance, and implications
• **Seasonality Insights**: Identify time-based patterns and their practical trading implications
• **Yield Curve Analysis**: Interpret rate structures, term relationships, and curve shapes

🎨 BEAUTIFUL PRESENTATION REQUIREMENTS:
• **PERCENTAGE FORMATTING**: Values are already percentages - add "%" only, don't multiply by 100
• **STATISTICAL PRECISION**: Use exact numbers from results (e.g., "correlation of 0.847", "volatility of 2.34%")
• **ELEGANT TABLES**: Create visually appealing tables with proper alignment and emojis
  | Asset Pair         | Correlation | Confidence | Interpretation    |
  |--------------------|-------------|------------|-------------------|
  | USDC-ETH          | 0.847       | ✅ High    | Strong positive   |
  | BTC-WETH          | -0.234      | ⚠️ Medium  | Weak negative     |
