...
export async function buildSchemaDoc() {
  ...
  for (const [fqtn, meta] of Object.entries(registry)) {
    ...
    // Optional extras (e.g., indexes, aliases) are not emitted to keep prompts lean
    lines.push('');
  }
  return lines.join('\n');
}
...
function finalizeAnswer(text /* questionForRange not used here */) {
  let out = String(text || '');
...
// Kept exported for testing/tools. Accept a doc string explicitly.
export function buildPrimaryPrompt(doc) {
  return [
    {
      role: 'system',
      content:
...
export function buildRetryPrompt(question, previousSql, errMsg, doc) {
  return [
    {
      role: 'system',
      content:
...
export function buildPlannerMessages(question, doc) {
  return [
    {
      role: 'system',
      content:
...
export async function planQuery(openai, question, doc) {
...
export async function retryPlan(openai, question, previousSql, errMsg, doc) {
  const resp = await openai.chat.completions.create({
    model: 'gpt-5',
    response_format: { type: 'json_object' },
    messages: buildRetryPrompt(question, previousSql, errMsg, doc),
  });
  const text = resp.choices?.[0]?.message?.content || '{}';
  let plan;
  try {
