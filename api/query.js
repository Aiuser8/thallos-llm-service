...
import { buildSchemaDoc, planQuery, retryPlan, generateAnswer } from '../lib/instructions.js';
...
export default async function handler(req, res) {
  try {
    ...
    // Optional auth via SERVICE_API_KEY
    ...
    // -------- Build whitelist doc from registry --------
    ...
    // Guard & run
    let plannedSql = String(plan.sql || '');
    let safeSql;
    try {
      // guardSql now enforces SELECT/limit; table whitelist relaxed per your preference
      safeSql = guardSql(plannedSql, new Set(), new Map(), MAX_LIMIT);
    } catch (e) {
