...
async function primaryPrompt(doc) {
...
async function retryPrompt(question, previousSql, errMsg, doc) {
  if (!doc) doc = await buildSchemaDoc();
  return [
    {
      role: 'system',
...
async function plannerMessages(question, doc) {
  if (!doc) doc = await buildSchemaDoc();
  return [
    {
      role: 'system',
...
export async function planQuery(openai, question, doc = null) {
  const msgs = await plannerMessages(question, doc);
...
export async function retryPlan(openai, question, previousSql, errMsg, doc = null) {
  const msgs = await retryPrompt(question, previousSql, errMsg, doc);
  const resp = await openai.chat.completions.create({
    model: 'gpt-5',
    response_format: { type: 'json_object' },
    messages: msgs,
