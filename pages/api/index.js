  ...
  return (
    <div style={{ maxWidth: 900, margin: '40px auto', fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, Arial' }}>
      ...
      <p style={{ color: '#555', marginTop: 0 }}>
        Model → SQL → guarded execution on Supabase → summary.
      </p>
      <form onSubmit={submit} style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        ...
        <button
          type="submit"
          disabled={loading}
          style={{ minWidth: 160, border: 0, borderRadius: 8, background: '#111', color: '#fff', padding: '12px 16px', cursor: 'pointer', height: 44 }}
        >
          {loading ? 'Running…' : 'Ask'}
        </button>
      </form>
